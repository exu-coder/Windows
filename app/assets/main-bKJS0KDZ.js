/* TYPING MASTER - Typing Engine v2 */
class TypingEngine {
  constructor() {
    this.text = "";
    this.currentIndex = 0;
  ...
  handleInput(char) {
    ...
    if (!isCorrect) {
      this.errors++;
      this.errorIndices.add(this.currentIndex);
  ...
  handleBackspace() {
    ...
    this.currentIndex--;
    this.errorIndices.delete(this.currentIndex);
    this.calculateAccuracy();
    return true;
  }
...
class App {
  ...
  updateSidebarCount() {
    ...
    if (el) {
      ...
      if (svg) {
        ...
        el.innerHTML = svgHTML + "Lessons (" + count + "/" + total + ")";
      }
    }
  }
  ...
  updateDisplay() {
    ...
    var errorIndices = this.engine.getErrorIndices();
